        ...
        <div class="bg-white relative border rounded-lg max-w-7xl m-auto">
                ...
                <table class="w-full text-sm text-left text-gray-500">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                        <tr>
                            ...
                            <th @click="sortBy('place')" class="px-4 py-3 cursor-pointer whitespace-nowrap">
                                Place <span v-if="sortKey === 'place'">{{ sortOrder === 'asc' ? '▲' : '▼' }}</span>
                            </th>
                            <th @click="sortBy('latitude')" class="px-4 py-3 cursor-pointer whitespace-nowrap">
                                Latitude <span v-if="sortKey === 'latitude'">{{ sortOrder === 'asc' ? '▲' : '▼' }}</span>
                            </th>
                            <th @click="sortBy('longitude')" class="px-4 py-3 cursor-pointer whitespace-nowrap">
                                Longitude <span v-if="sortKey === 'longitude'">{{ sortOrder === 'asc' ? '▲' : '▼' }}</span>
                            </th>
                            <th @click="sortBy('kmDepth')" class="px-4 py-3 cursor-pointer whitespace-nowrap">
                                Km. Depth <span v-if="sortKey === 'kmDepth'">{{ sortOrder === 'asc' ? '▲' : '▼' }}</span>
                            </th>
                            <th @click="sortBy('earthquakeTime')" class="px-4 py-3 cursor-pointer whitespace-nowrap">
                    ...
                    <tbody>
                        <tr v-if="loading">
                            <td class="py-8 text-center" colspan="7">
                        ...
                        <tr v-else
                            v-for="item in filteredItems.slice((currentPage - 1) * itemsPerPage, (itemsPerPage * currentPage))"
                            :key="item.id" class="border-b border-gray-200">
                            <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">{{ item.magnitude.toFixed(2) }}</td>
                            <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">{{ item.place }}</td>
                            <td class="px-4 py-3 whitespace-nowrap">{{ item.latitude.toFixed(3) }}°N</td>
                            <td class="px-4 py-3 whitespace-nowrap">{{ item.longitude.toFixed(3) }}°W</td>
                            <td class="px-4 py-3 whitespace-nowrap">{{ item.kmDepth }} km</td>
                            <td class="px-4 py-3 whitespace-nowrap">{{ new Date(item.earthquakeTime).toUTCString() }}</td>
            ...
            <div v-if="totalPages > 1" class="mt-4 mb-2 flex justify-center items-center gap-1">
                ...
                <ItemsPerPageDropdown :items-per-page="itemsPerPage"
                    @items-per-page-change="handleItemsPerChangeChange" />
            </div>
        </div>
    </div>
</template>